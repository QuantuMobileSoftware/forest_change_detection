...
def parse_args():
    ...
    parser.add_argument(
        '--neighbours', '-nbr', dest='neighbours', default=5,
...
def imgcube(df, I, neighbours, save_path, data_path, img_path, msk_path, cloud_path, writer, width,height):
    ...
    for i in range(len(xs)):
        ...
        msks = [ ((gaussian_filter( ( msks[i]-msks[-1] ), 0.5)>0)*255).astype(np.uint8) for i in range(len(msks))]
        #msks = [ ((msks[i]-msks[-1]>0)*255).astype(np.uint8) for i in range(len(msks))]
        if len(imgs) == len(msks) and len(imgs)==neighbours:
            ...
            for msk in msks:
                u+=1
                imageio.imwrite(os.path.join(diff_path, msk_path, diff_path.split('/')[-1]+'_'+xs[i]+'_'+ys[i]+f'_{u}.png'), msk)
            
            writer.writerow([
                #diff_path.split('/')[-1], diff_path.split('/')[-1], xs[i]+'_'+ys[i], int(np.sum(diff_msk)/255)
                diff_path.split('/')[-1], diff_path.split('/')[-1], xs[i]+'_'+ys[i], int(np.sum(msks)/255)
            ])
        else: pass
